...
class Site(models.Model):
    ...
    @staticmethod
    def get_populated_site(site_name, texts=None, images=None, files=None, paragraph_lists=None, clear_redundant=False):
        """
        ...
        current_file_keys = SiteFile.objects.filter(site=site).values_list('key', flat=True)
        ...
        if clear_redundant:
            delete_file_keys = [file_key for file_key in current_file_keys if file_key not in files]
            SiteFile.objects.filter(key__in=delete_file_keys, site=site).delete()

        for file_key in [file_key for file_key in files if file_key not in current_file_keys]:
            site_file = SiteFile(key=file_key, site=site)
            try:
                site_file.full_clean()
            except ValidationError:
                raise ValidationError(
                    "SiteImage with key '%s' of site '%s' failed to validate." % (file_key, str(site_file.site))
                )
...
class SiteFile(models.Model):
    """ Model representing an file connected to a site. """
    key = models.CharField(max_length=100, blank=False, null=False, validators=[validate_variable_name])
    file = models.FileField(null=False, blank=True)
    site = models.ForeignKey(Site, on_delete=models.CASCADE, blank=False, null=False, related_name='files')
